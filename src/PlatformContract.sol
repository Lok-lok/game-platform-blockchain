...
contract PlatformContract {
    ...
    event tradeEvent (
        uint indexed itemId, uint indexed itemPrice
    );
    ...
    function getOfferCount() public view returns (uint8) {
     require(isUser(msg.sender));
   ...
   // require(isUser(msg.sender));
   // require(i>=0 && i< globalOfferId);
    return offers[offerId].itemId;
    }
    
    function getOfferName(uint8 offerId) public view returns (string memory){
    return getItemName(getOfferItemId(offerId));
    }
    
    function getOfferSellerId(uint8 offerId) public view returns (address) {
    ...
    function getUserItemCount() public view returns (uint8){
    return users[msg.sender].itemCount;
    }
    
    
    function getUserItem(uint8 index) public view returns (uint8){
    ...
    function isValidItemId(uint8 itemId) public view returns (bool) {
        return itemId > 0 && itemId < globalItemId+1;
    }
